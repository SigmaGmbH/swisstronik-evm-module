...
func (k Keeper) NodePublicKey(c context.Context, _ *types.QueryNodePublicKey) (*types.QueryNodePublicKeyResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	nodePublicKey, err := k.GetNodePublicKey(ctx)
	if err != nil {
		return nil, err
	}
