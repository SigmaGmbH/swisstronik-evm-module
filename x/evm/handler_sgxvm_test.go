...
// DoSGXVMSetupTest setup test environment, it uses`require.TestingT` to support both `testing.T` and `testing.B`.
func (suite *EvmTestSuite) DoSGXVMSetupTest(t require.TestingT) {
	checkTx := false
...
func (suite *EvmTestSuite) SetupSGXVMTest() {
	suite.DoSGXVMSetupTest(suite.T())
}

func (suite *EvmTestSuite) SignSGXVMTx(tx *types.MsgHandleTx) {
	tx.From = suite.from.String()
	err := tx.Sign(suite.ethSigner, suite.signer)
	suite.Require().NoError(err)
}

func (suite *EvmTestSuite) TestHandleMsgHandleTx() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				tx = types.NewSGXVMTx(suite.chainID, 0, &to, big.NewInt(100), 10_000_000, big.NewInt(10000), nil, nil, nil, nil)
...
func (suite *EvmTestSuite) TestSGXVMHandlerLogs() {
	// Test contract:
	...
	tx := types.NewSGXVMTx(suite.chainID, 1, nil, big.NewInt(0), gasLimit, gasPrice, nil, nil, bytecode, nil)
	suite.SignSGXVMTx(tx)
...
func (suite *EvmTestSuite) TestSGXVMDeployAndCallContract() {
	// Test contract:
	...
	tx = types.NewSGXVMTx(suite.chainID, 2, &receiver, big.NewInt(0), gasLimit, gasPrice, nil, nil, bytecode, nil)
	suite.SignSGXVMTx(tx)
...
func (suite *EvmTestSuite) TestSGXVMSendTransaction() {
	gasLimit := uint64(21000)
	gasPrice := big.NewInt(0x55ae82600)

	// send simple value transfer with gasLimit=21000
	tx := types.NewSGXVMTx(suite.chainID, 1, &common.Address{0x1}, big.NewInt(1), gasLimit, gasPrice, nil, nil, nil, nil)
	suite.SignSGXVMTx(tx)
...
func (suite *EvmTestSuite) TestSGXVMOutOfGasWhenDeployContract() {
	// Test contract:
	//http://remix.ethereum.org/#optimize=false&evmVersion=istanbul&version=soljson-v0.5.15+commit.6a57276f.js
	//2_Owner.sol
	//
...
func (suite *EvmTestSuite) TestSGXVMErrorWhenDeployContract() {
...
func (suite *EvmTestSuite) deploySGXVMERC20Contract() common.Address {
...
func (suite *EvmTestSuite) TestSGXVMERC20TransferReverted() {
	...
	for _, tc := range testCases {
		suite.Run(tc.msg, func() {
			...
			contract := suite.deploySGXVMERC20Contract()
			...
			suite.Require().NoError(err)
			fees, err := keeper.VerifyFee(txData, "swtr", baseFee, true, true, suite.ctx.IsCheckTx())
			suite.Require().NoError(err)
			err = k.DeductTxCostsFromUserBalance(suite.ctx, fees, common.HexToAddress(tx.From))
			suite.Require().NoError(err)

			res, err := k.HandleTx(sdk.WrapSDKContext(suite.ctx), tx)
			suite.Require().NoError(err)
...
func (suite *EvmTestSuite) TestSGXVMContractDeploymentRevert() {
	...
	for _, tc := range testCases {
		suite.Run(tc.msg, func() {
			...
			rsp, err := k.HandleTx(sdk.WrapSDKContext(suite.ctx), tx)
			suite.Require().NoError(err)
